...
// Example usage:
// unsigned short sensor_values[8];
// sensors.read(sensor_values);
// The values returned are a measure of the reflectance in abstract units,
// with higher values corresponding to lower reflectance (e.g. a black
// surface or a void).
void QTRSensors::read(unsigned short *sensor_values, unsigned char readMode)
{
  unsigned short off_values[QTR_MAX_SENSORS];
  unsigned char i;
...
void QTRSensors::calibrateOnOrOff(unsigned short **calibratedMinimum,
                    unsigned short **calibratedMaximum,
                    unsigned char readMode)
{
  short i;
  unsigned short sensor_values[16];
  unsigned short max_sensor_values[16];
  unsigned short min_sensor_values[16];

  // Allocate the arrays if necessary.
  if(*calibratedMaximum == 0)
  {
    *calibratedMaximum = (unsigned short*)malloc(sizeof(unsigned short)*_numSensors);
  ...
  {
    *calibratedMinimum = (unsigned short*)malloc(sizeof(unsigned short)*_numSensors);
...
void QTRSensors::readCalibrated(unsigned short *sensor_values, unsigned char readMode)
{
  short i;
...
short QTRSensors::readLine(unsigned short *sensor_values,
  unsigned char readMode, unsigned char white_line)
{
  unsigned char i, on_line = 0;
  unsigned long avg; // this is for the weighted total, which is long
                     // before division
  unsigned short sum; // this is for the denominator which is <= 64000
  static short last_value=0; // assume initially that the line is left.
...
QTRSensorsRC::QTRSensorsRC(unsigned char* pins,
  unsigned char numSensors, unsigned short timeout, unsigned char emitterPin)
{
  calibratedMinimumOn = 0;
  calibratedMaximumOn = 0;
  calibratedMinimumOff = 0;
...
void QTRSensorsRC::readPrivate(unsigned short *sensor_values)
{
  unsigned char i;
  ...
  {
    unsigned short time = micros() - startTime;
    for (i = 0; i < _numSensors; i++)
    {
...
void QTRSensorsAnalog::readPrivate(unsigned short *sensor_values)
{
  unsigned char i, j;
