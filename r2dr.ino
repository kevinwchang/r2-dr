...
const int16_t DomeSensorMin = 100;
...
void loop()
{
  ...
  switch (state)
  {
    case WaitForButton:
      ...
      if (!btn.getSingleDebouncedRelease()) { return; }
...
{
  static int8_t lastLCount = 0, lastRCount = 0;
  static uint8_t leapLCount = 0, leapRCount = 0;
  
  int8_t dLCount = (int8_t)(lwEnc.read() - lastLCount);
  int8_t dRCount = (int8_t)(rwEnc.read() - lastRCount);

  Serial.print(dLCount);
  Serial.print('\t');
  Serial.println(dRCount);
  ...
  /*static uint16_t lastProportionalrint = 0;
  if ((uint16_t)(millis() - lastProportionalrint) > 500)
  {
    printDebug();
    lastProportionalrint = millis();
  }*/
}
